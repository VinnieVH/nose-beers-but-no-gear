...
class WowAPI {
  ...
  private readonly clientId = import.meta.env.VITE_BLIZZARD_API_CLIENT_ID
  private readonly clientSecret = import.meta.env.VITE_BLIZZARD_API_CLIENT_SECRET

  constructor() {
    if (!this.clientId || !this.clientSecret) {
      console.warn('Blizzard API credentials not found. Please set VITE_BLIZZARD_CLIENT_ID and VITE_BLIZZARD_CLIENT_SECRET environment variables.')
    }
  }
...
// Export convenient functions with default parameters
export const fetchGuildInfo = async (
  ...
  return wowApi.fetchGuild(realmSlug, guildSlug, region)
}
...
export const fetchGuildRoster = async (
  ...
  const realmSlug = getRealmSlug(realmName)
  const guildSlug = getGuildSlug(guildName)
  return wowApi.fetchGuildRoster(realmSlug, guildSlug, region)
}

export const fetchGuildAchievements = async (
  guildName = GUILD_NAME,
  realmName = GUILD_REALM,
  region = GUILD_REGION
) => {
  const realmSlug = getRealmSlug(realmName)
  const guildSlug = getGuildSlug(guildName)
  return wowApi.fetchGuildAchievements(realmSlug, guildSlug, region)
}

export const fetchGuildActivity = async (
  guildName = GUILD_NAME,
  realmName = GUILD_REALM,
  region = GUILD_REGION
) => {
  const realmSlug = getRealmSlug(realmName)
  const guildSlug = getGuildSlug(guildName)
  return wowApi.fetchGuildActivity(realmSlug, guildSlug, region)
}
...
export const fetchAllGuildData = async (
  ...
  const realmSlug = getRealmSlug(realmName)
  const guildSlug = getGuildSlug(guildName)
  return wowApi.fetchAllGuildData(realmSlug, guildSlug, region)
}
